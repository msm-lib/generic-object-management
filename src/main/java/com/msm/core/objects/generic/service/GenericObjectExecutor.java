...
import com.msm.core.objects.generic.GenericObjectUtils;
...
//        Utils.CL.emptyIfNull(pageResponse.getContents()).forEach(this::preprocessObject);
        ...
        PageResponse<Object> pageResponse = dynamicQueryService.query(request.getPayload());
        Utils.CL.emptyIfNull(pageResponse.getContents()).forEach(this::preprocessObject);
...
//        List<Object> objects = pageResponse.getContents();
        ...
        List<Object> objects = pageResponse.getContents();
        Utils.CL.emptyIfNull(objects).forEach(this::preprocessObject);
        if(Utils.CL.isNotEmpty(objects)) {
            return (X) objects.getFirst();
        }
        return null;
    }
    ...
    public <X> X create(ActionRequest<Map<String, Object>> request) {
        ...
        GenericObjectUtils.updateRef(objectMetadata, request.getPayload());
        return (X) dynamicQueryService.insertReturning(objectMetadata, request.getPayload());
...
//        return jpaRepository.save(oldData);
    ...
    public <X> X delete(ActionRequest<Map<String, Object>> request) {
        ObjectMetadata objectMetadata = getObjectMetadata(request.getObjectName());
        dynamicQueryService.deleteById(objectMetadata, request.getObjectId());
...
//    }
    ...
    private ObjectMetadata getObjectMetadata(String objectName) {
        return ObjectMetadataContextHolder
