...
import com.msm.core.objects.integration.auth.oauth2password.CachedOAuth2PasswordTokenManager;
import com.msm.core.objects.integration.auth.oauth2password.OAuth2PasswordProvider;
...
public class AuthProviderUtils {
    public static AuthProvider createProvider(ConnectorProperties config, RequestClient restClient, RetryExecutor retryExecutor) {
        ...
        return switch (authProviderProperties.getType()) {
            ...
            case "oauth2-password" -> createOAuth2PasswordProvider(
                    retryExecutor,
                    authProviderProperties,
                    restClient
            );
    ...
    private static AuthProvider createOAuth2Provider(AuthProviderProperties config, RequestClient restClient) {
        TokenProvider tokenManager = new CachedOAuth2TokenManager(restClient, config.getOauth2());
        return new OAuth2AuthProvider(tokenManager);
    }

    private static AuthProvider createOAuth2PasswordProvider(
            RetryExecutor retryExecutor,
            AuthProviderProperties config,
            RequestClient restClient) {

        TokenProvider tokenManager = new CachedOAuth2PasswordTokenManager(restClient, retryExecutor, config.getOauth2Password());
        return new OAuth2PasswordProvider(tokenManager);
    }
}
